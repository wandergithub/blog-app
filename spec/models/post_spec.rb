...
RSpec.describe Post, type: :model do
  describe 'Post validation' do
    ...
    it 'does not allow non integer values for :likes_counter' do
      expect(Post.new(title: 'My post', comments_counter: 0, likes_counter: 'Not a valid type: string', author: User.new)).to be_invalid
      expect(Post.new(title: 'My post', comments_counter: 0, likes_counter: 0, author: User.new)).to be_valid
    end
    ...
    it 'Has a a most_recent_comments method' do
      expect(Post.new(title: 'My post', comments_counter: 0, likes_counter: 0, author: User.new).most_recent_comments).to be_truthy
    end
  end
end